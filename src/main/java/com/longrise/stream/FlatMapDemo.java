...
public class FlatMapDemo {
  public static void main(String[] args) {
    ...
    System.out.println("-------------");
    // 生成6位 int 类型验证码
    int rs = random.ints().filter(i -> i > 0 && String.valueOf(i).length() == 6).findAny().getAsInt();
    System.err.println("rs=" + rs);
    // 生成6位 int 类型验证码(简单)
    rs = random.ints(100000, 1000000).findAny().getAsInt();
    System.err.println("rs=" + rs);

    // 使用 ints 方法生成 IntStream, 在通过 boxed 方法包装成 Stream<Integer>, 再通过 map 方法获取属性并包装成
    // Stream<String>, 最后取值(默认值000000)
    // 生成6位 String 类型验证码
    String orElseGet = random.ints().filter(i -> i > 0 && String.valueOf(i).length() == 6).boxed().map(String::valueOf)
        .findAny().orElseGet(() -> "000000");
    System.err.println("orElseGet=" + orElseGet);
    // 生成6位 String 类型验证码
    orElseGet = random.ints(100000, 1000000).boxed().map(String::valueOf).findAny().orElseGet(() -> "000000");
    System.err.println("orElseGet=" + orElseGet);
